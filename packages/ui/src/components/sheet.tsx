...
// No closed-state animate-out/slide-out on the content either: while open, this
// modal layer sets `document.body.style.pointerEvents = "none"` (via Radix's
// DismissableLayer), and that lock is only released when the node unmounts.
// Radix's Presence gates unmount on an `animationend` event that iOS WebKit can
// silently drop — so an exit animation here risks leaving the whole page's taps
// dead. With no exit animation, Presence unmounts synchronously on close and the
// body lock is always released. Entrance animation (data-[state=open]) is kept.
