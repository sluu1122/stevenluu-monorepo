...
import { CPT_CODES } from './data/cpt-codes.js';
...
// Evict fully-expired entries once per window so the Map doesn't grow unbounded
setInterval(() => {
  const now = Date.now();
...
export function createServer() {
  ...
  app.get('/api/cpt/search', (req, res) => {
    ...
    const results = q
      ? CPT_CODES.filter(c =>
          c.code.startsWith(q) || c.description.toLowerCase().includes(q)
        ).slice(0, limit)
      : CPT_CODES.slice(0, limit);
