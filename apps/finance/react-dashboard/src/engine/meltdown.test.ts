...
describe('calculateMeltdownWithdrawal', () => {
  it('is a no-op when disabled', () => {
    ...
    const result = calculateMeltdownWithdrawal(rule({ enabled: false }), 2026, 30_000, buckets, balances);
    expect(result.totalWithdrawn).toBe(0);
  });

  it('is a no-op when undefined (unset scenario field)', () => {
    const balances = { rrsp: 200_000 };
    const result = calculateMeltdownWithdrawal(undefined, 2026, 30_000, buckets, balances);
    expect(result.totalWithdrawn).toBe(0);
  });

  it('withdraws exactly enough to reach the ceiling', () => {
    const balances = { rrsp: 200_000 };
    const result = calculateMeltdownWithdrawal(rule(), 2026, 30_000, buckets, balances);
  ...
  it('does nothing once taxable income already meets or exceeds the ceiling', () => {
    const balances = { rrsp: 200_000 };
    const result = calculateMeltdownWithdrawal(rule(), 2026, 60_000, buckets, balances);
    expect(result.totalWithdrawn).toBe(0);
  });

  it('caps at the available balance rather than overdrawing', () => {
    const balances = { rrsp: 5_000 };
    const result = calculateMeltdownWithdrawal(rule(), 2026, 0, buckets, balances);
    expect(result.totalWithdrawn).toBe(5_000);
  });

  it('does nothing outside the configured year window', () => {
    const balances = { rrsp: 200_000 };
    const before = calculateMeltdownWithdrawal(rule({ startYear: 2030 }), 2026, 0, buckets, balances);
    const after = calculateMeltdownWithdrawal(rule({ endYear: 2020 }), 2026, 0, buckets, balances);
