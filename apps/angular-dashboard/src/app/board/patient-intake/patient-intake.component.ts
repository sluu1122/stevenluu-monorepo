...
export class PatientIntakeComponent {
  protected readonly wizardStore  = inject(IntakeWizardStore);
  ...
  protected readonly modalTitle = computed(() => {
    switch (this.wizardStore.modalKind()) {
  ...
  onEscape(): void {
    if (this.wizardStore.modalKind()) this.wizardStore.closeModal();
  }

  protected openModal(kind: Parameters<typeof this.wizardStore.openModal>[0]): void {
    this.wizardStore.openModal(kind);
  }

  protected closeModal(): void { this.wizardStore.closeModal(); }

  protected onOverlayClick(event: MouseEvent): void {
    if ((event.target as HTMLElement) === event.currentTarget) this.wizardStore.closeModal();
  }
}
